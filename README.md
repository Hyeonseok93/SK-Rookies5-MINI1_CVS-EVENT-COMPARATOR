...
별도 DB 없이 **CSV 카탈로그**를 중심으로 동작합니다. 4사 크롤러가 수집한 상품은 공통 스키마(`brand`, `name`, `price`, `event`, `img_url`)로 정규화되어 `data/`에 **raw → cleaned → categorized** 3단계로 적재됩니다. 배치가 매일 이 파이프라인을 돌려 대시보드가 읽는 카탈로그를 갱신하고, 챗봇은 분류된 카탈로그를 컨텍스트로 사용합니다.

> 브랜드별 크롤링 전략과 수집 파이프라인 설계는 [기술 블로그](https://bulldog93.tistory.com/45)에서 다룹니다.
